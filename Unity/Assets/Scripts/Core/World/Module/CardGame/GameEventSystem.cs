...
{
    [Code]
    public class GameEventSystem: Singleton<GameEventSystem>, ISingletonAwake
    {
        private class GameEventInfo
        {
            public IGameEventType IEvent { get; }

            public GameEventInfo(IGameEventType iEvent)
            {
                this.IEvent = iEvent;
            }
        }
        
        private readonly Dictionary<Type, List<GameEventInfo>> allEvents = new();
        ...
        public void Awake()
        {
            ...
            foreach (Type type in codeTypes.GetTypes(typeof (GameEventAttribute)))
            {
                IGameEventType obj = Activator.CreateInstance(type) as IGameEventType;
                if (obj == null)
                {
                    throw new Exception($"type not is GameEvent: {type.Name}");
                }
                
                object[] attrs = type.GetCustomAttributes(typeof(GameEventAttribute), false);
                foreach (object attr in attrs)
                {
                    GameEventAttribute eventAttribute = attr as GameEventAttribute;

                    GameEventInfo eventInfo = new(obj);

                    Type eventType = obj.GetType();

                    if (!this.allEvents.ContainsKey(eventType))
                    {
                        this.allEvents.Add(eventType, new List<GameEventInfo>());
                    }
                    //Log.Warning("增加GameEventType:" + eventType.ToString());
        ...
        {
            List<GameEventInfo> iEvents;
            Log.Warning("发送消息---1");
            Log.Warning(typeof (T).ToString());
            ...
            foreach (GameEventInfo eventInfo in iEvents)
            {
                if (!(eventInfo.IEvent is GameEvent<S, T> aEvent))
                {
                    Log.Warning("发送消息---1");
